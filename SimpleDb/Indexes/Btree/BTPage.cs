...
namespace SimpleDb.Indexes.Btree
{
    ...
    public class BTPage
    {
        ...
        {
            Debug.Assert(this.currentblk != null);
            tx.SetValue(currentblk.Value, 0, val, true);
        }
        ...
        {
            tx.SetValue(blk, 0, flag, false);
            tx.SetValue(blk, sizeof(int), 0, false);  // #records = 0
            ...
            foreach (String fldname in layout.schema().ColumnNames())
            {
                ...
                switch (layout.schema().GetSqlType(fldname))
                {
                    case SqlType.INTEGER:
                        tx.SetValue(blk, pos + offset, 0, false);
                        break;
                    case SqlType.VARCHAR:
                        tx.SetValue(blk, pos + offset, "", false);
                        break;
                    case SqlType.DATETIME:
                        tx.SetValue(blk, pos + offset, new DateTime(), false);
                        break;
                    default:
        ...
        private DbString getString(int slot, String fldname)
        {
            int pos = fldpos(slot, fldname);
            Debug.Assert(this.currentblk != null);
        ...
        private Constant getVal(int slot, String fldname)
        {
            ...
            {
                case SqlType.INTEGER:
                    return getInt(slot, fldname);
                case SqlType.VARCHAR:
                    return getString(slot, fldname);
                case SqlType.DATETIME:
                    return getDateTime(slot, fldname);
                default:
                    throw new NotImplementedException();
            }
        }
        ...
        private void setInt(int slot, String fldname, int val)
        {
            int pos = fldpos(slot, fldname);
            Debug.Assert(this.currentblk != null);
            tx.SetValue(currentblk.Value, pos, val, true);
        }

        private void setString(int slot, String fldname, DbString val)
        {
            int pos = fldpos(slot, fldname);
            Debug.Assert(this.currentblk != null);
            tx.SetValue(currentblk.Value, pos, val, true);
        ...
        {
            Debug.Assert(this.currentblk != null);
            tx.SetValue(currentblk.Value, sizeof(int), n, true);
        }
